"""
extract_effectiveness.py — Standalone Effectiveness extraction script
======================================================================
Run this to extract only Effectiveness source data without running the full pipeline.

Usage:
    python extract_effectiveness.py
    python extract_effectiveness.py --max-pages 50
    python extract_effectiveness.py --mode laptop --api-key sk-...
    python extract_effectiveness.py --clear-cache
"""
...
def _build_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone Effectiveness PDF extraction")
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir).resolve() if args.output_dir else _START_DIR / "output" / "effectiveness_cache"
    if args.clear_cache and output_dir.exists():
        print(f"  Clearing Effectiveness cache: {output_dir}")
        shutil.rmtree(output_dir)

    print("\n" + "=" * 64)
    print("  STANDALONE EFFECTIVENESS EXTRACTION")
    print(f"  Max pages per PDF : {max_pages if max_pages != -1 else 'ALL'}")
    print("=" * 64)

    df = run_effectiveness_extraction(
    ...
    print(f"\n  Effectiveness extraction complete — {len(df)} rows")
