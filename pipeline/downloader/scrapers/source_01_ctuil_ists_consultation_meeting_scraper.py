...
Output layout:
    source_output/CTUIL-ISTS-CMETS/agenda/<Region>/   — Agenda PDFs
    source_output/CTUIL-ISTS-CMETS/minutes/<Region>/  — Minutes PDFs
"""
...
def safe_folder_name(name: str) -> str:
    ...
    return cleaned or "Unknown Region"
...
async def download(session, url, dest, *, region=None, doc_type=None):
    async with DOWNLOAD_SEM:
        ...
        cache_type = f"{doc_type}/{region}" if region else doc_type
        if os.path.exists(dest):
            cache.record_download(pdf_name=pdf_name, pdf_type=cache_type, pdf_path=dest)
            return
        if cache.is_cached(pdf_name, pdf_type=cache_type, pdf_path=dest):
            return
...
async def main():
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        ...
        for rec in filtered:
            grouped[(rec["doc_type"], rec["region"])].append(rec)

        tasks = []

        for (dtype, region), records in grouped.items():
            # Route Agenda -> agenda/<Region>/  |  Minutes -> minutes/<Region>/
            sub_folder = dtype.lower()     # "agenda" or "minutes"
            region_folder = safe_folder_name(region)
            dest_dir   = ensure_dir(OUTPUT_DIR, sub_folder, region_folder)
